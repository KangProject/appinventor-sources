...
public class File extends AndroidNonvisibleComponent implements Component {
   ...
   */
  @SimpleFunction
  public void OverwriteTo(final String text, final String fileName) {
      byte[] array = text.getBytes();
      InputStream in = new ByteArrayInputStream(array);
      File filename = FileUtil.getFile(fileName, "txt");
      try {       
          writeFile(in, filename);
          in.close();
          } catch (IOException e) {
                  form.dispatchErrorOccurredEvent(this, "WriteTo",
                                  ErrorMessages.ERROR_UNABLE_TO_WRITE_TO_FILE, filename);
          }
  }
  ...
  public void ReadFrom(String fileName) {
        File filename = FileUtil.getFile(fileName, "txt");
        try {
          byte[] viewArray = FileUtil.readFile(filename);
          text = new String(viewArray);
          } catch (IOException e) {
                  form.dispatchErrorOccurredEvent(this, "ReadFrom",
                                  ErrorMessages.ERROR_UNABLE_TO_READ_FROM_FILE, filename);
          }
      return text;
  }
  ...
  public void Delete(String fileName) {
          ...
          if (! success) {
                  form.dispatchErrorOccurredEvent(this, "Delete",
                                  ErrorMessages.ERROR_UNABLE_TO_DELETE_FILE, file());
          }
    }
  }
}
